...
def get_options(args=None):
    ...
    if options.net_file is None or options.measurment_file is None or options.route_file is None or \
       options.interval_size is None or options.route_file is None or options.edgedata_nx is None:
...
def update_option(option):
    ...
    with open(option.output_location + 'option.txt', 'w') as f:
...
def route_sampling(od, _routes, option, interval,  iteration, sample_iteration):
    ...
    text3 = """\n</routes>"""
    with open(option.output_location+str(interval)+"/"+str(iteration)+"_"+str(sample_iteration)+"_routes_sim.xml", 'w') as myfile: 
...
"""
    ...
    text3 = """\n</additional>"""
    with open(option.output_location+str(interval)+"/"+str(iteration)+"_"+str(sample_iteration)+"_edge_data_add.xml", 'w') as myfile: 
        myfile.write(text0+text1+text2+text3)
...
def sumoCommand(option, interval, iteration, sample_iteration):
    ...
    sc_dict["--route-files"] = diroutput+ str(iteration)+"_"+str(sample_iteration)+"_routes_sim.xml"
    sc_dict["--additional-files"] = diroutput + str(iteration) +"_"+str(sample_iteration)+ "_edge_data_add.xml"
    sc_dict["--tripinfo-output"] = diroutput + str(iteration)+"_"+str(sample_iteration)+"_trip_info.xml"
    sc_dict["--statistic-output"] = diroutput + str(iteration)+"_"+str(sample_iteration)+"_statistic_output.xml"
    sc_dict["--vehroute-output"] =  diroutput + str(iteration)+"_"+str(sample_iteration)+"_vehroute_output.xml"
...
def simulation(option, interval, iteration, dod, last_best_iteration, last_best_sample_iteration, net, sample_iteration):
    ...
    traci.start(sumoCmd)
    savedstate = option.output_location+str(interval-1)+"/"+str(last_best_iteration)+"_"+str(last_best_sample_iteration)+"_savedstate.xml"
    ...
    while step < int(option.interval_size):
     ...
     #       tripset.add((_from, _to))         
        ...
        step += 1
    state = traci.simulation.saveState(option.output_location+str(interval)+"/"+str(iteration)+"_"+str(sample_iteration)+"_savedstate.xml")
...
def fun_edge2time(net, option, interval, iteration, sample_iteration):
    sumolib.subprocess.call(["python3", option.tools+"/xml/xml2csv.py", option.output_location+ str(interval)+"/"+str(iteration)+"_"+str(sample_iteration)+"_aggregated.xml",
                            "--xsd",option.sumo_home + "/data/xsd/meandata_file.xsd"])
    aggregated = pd.read_csv(option.output_location+str(interval)+"/"+str(iteration)+"_"+str(sample_iteration)+"_aggregated.csv", sep=";")
...
def shortest_path_OD(option, edge2time, interval, iteration, best_sample):
    ...
    G = nx.from_pandas_edgelist(edges_nx, source="from", target="to", edge_attr=True, create_using=nx.DiGraph)
    id2type = edges_nx.set_index("id")["type"] 
    manager = multiprocessing.Manager()
    return_dict = manager.dict()
    processes = []
    sumolib.subprocess.call(["python3", option.tools + "/xml/xml2csv.py", option.output_location+str(interval)+"/"+str(iteration)+"_"+str(best_sample)+"_routes_sim.xml"])
    routes_w = pd.read_csv(option.output_location+str(interval)+"/"+str(iteration)+"_"+str(best_sample)+
...
def route_update(_routes, option, edge2id, edge2time, interval=-1, iteration=-1, best_sample=-1):
    ...
    _interval = interval
    #######################################################
    #_interval = -1    # for statics routea table !!!!!!!!!!
    #######################################################
    if _interval!=-1:
...
def update_sensor_extra(all_routes, sensor_set, option, interval, iteration, sample_iteration):
    ...
    sumolib.subprocess.call(["python3", option.tools+"/xml/xml2csv.py", option.output_location+str(interval)+"/"+str(iteration)+"_"+str(sample_iteration)+"_vehroute_output.xml"])
    sumolib.subprocess.call(["python3", option.tools+"/xml/xml2csv.py", option.output_location+str(interval)+"/"+str(iteration)+"_"+str(sample_iteration)+"_trip_info.xml"])

    vehroute = pd.read_csv(option.output_location+str(interval)+"/"+str(iteration)+"_"+str(sample_iteration)+"_vehroute_output.csv", sep=";")
    vehroute["vehicle_arrival"] = vehroute["vehicle_arrival"].fillna(-1)
    tripinfo = pd.read_csv(option.output_location+str(interval)+"/"+str(iteration)+"_"+str(sample_iteration)+"_trip_info.csv", sep=";")
...
def logging_statistics(option, interval, iteration, sample_iteration):
    sumolib.subprocess.call(["python3", option.tools+"/xml/xml2csv.py", option.output_location+str(interval)+"/"+str(iteration)+"_"+str(sample_iteration)+"_trip_info.xml"])
    trip_info = pd.read_csv(option.output_location+str(interval)+"/"+str(iteration)+"_"+str(sample_iteration)+"_trip_info.csv", sep=";")
    ...
    tree = ET.parse(option.output_location+str(interval)+"/"+str(iteration)+"_"+str(sample_iteration)+"_statistic_output.xml") 
    ...
    vehicleTripStatistics["teleport"] = root[1].attrib["total"]
...
def viz_edge_count(routes, od, net, option, interval, iteration, sample_iteration, geo=True):
    ...
    edges_count_data.to_csv(option.output_location+str(interval)+"/" +str(iteration)+"_"+str(sample_iteration)+"_edge_count_viz.csv", index=False)
...
def fun_sensor_error(option, interval, iteration, sample_iteration):
    ...
    interval_size=int(option.interval_size)
    sumolib.subprocess.call(["python3", option.tools+"/xml/xml2csv.py", output_dir+str(interval)+"/"+str(iteration)+"_"+str(sample_iteration)+"_vehroute_output.xml"])
...
def clean_samples(option, interval, iteration, sample):
    allfiles = []
    path = option.output_location+str(interval)+"/"
    for i in range(int(option.sample_iteration)):
        if i!=sample:
            required_files = glob.glob(path+str(iteration)+"_"+str(i)+"_*") # This gives all the files that matches the pattern
...
def clean_iteration(option, interval, iteration):
    path = option.output_location+str(interval)+"/"
    required_files = glob.glob(path+str(iteration)+"_*") # This gives all the files that matches the pattern
...
def calculate_OD_waiting(option, interval, iteration, sample_iteration):
    ...
    tripinfo_w = pd.read_csv(option.output_location+str(interval)+"/"+str(iteration)+"_"+str(sample_iteration)+
                            "_trip_info.csv", sep=";")

    sumolib.subprocess.call(["python3", option.tools+"/xml/xml2csv.py", option.output_location+str(interval)+"/"+str(iteration)+"_"+str(sample_iteration)+"_routes_sim.xml"])

    routes_w = pd.read_csv(option.output_location+str(interval)+"/"+str(iteration)+"_"+str(sample_iteration)+
                        "_routes_sim.csv", sep=";")
...
def main():
    ...
    real_data = pd.read_csv(option.output_location+"real_data.csv")
    routes = pd.read_csv(option.output_location+"route_file.csv", low_memory=False)
    ...
    for interval in range(int(option.interval_begin), int(option.interval_end)+1):
        ...
        od_waiting.to_csv(option.output_location+str(interval) +"_od_waiting.csv", index=False)
        #print(od_waiting)
